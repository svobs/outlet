...
from command.command_interface import Command, CommandContext, CommandResult, CommandStatus, CopyNodeCommand, DeleteNodeCommand, TwoNodeCommand
...
class CopyFileLocallyCommand(CopyNodeCommand):
    ...
    def execute(self, cxt: CommandContext) -> CommandResult:
        ...
        # update cache:
        local_node = cxt.cache_manager.build_local_file_node(full_path=dst_path)
        assert local_node.uid == self.change_action.dst_node.uid, f'LocalNode={local_node}, DstNode={self.change_action.dst_node}'
...
class DeleteLocalFileCommand(DeleteNodeCommand):
    ...
    def __repr__(self):
        return f'{__class__.__name__}(uid={self.identifier}, total_work={self.get_total_work()}, to_trash={self.to_trash}, ' \
               f'delete_empty_parent={self.delete_empty_parent}, status={self.status()}, tgt_node={self.change_action.src_node}'
...
class MoveFileLocallyCommand(TwoNodeCommand):
    ...
    def execute(self, cxt: CommandContext):
        ...
        if not os.path.exists(self.change_action.src_node.full_path):
            to_delete = [self.change_action.src_node]
    ...
    def __repr__(self):
        return f'{__class__.__name__}(uid={self.identifier}, total_work={self.get_total_work()}, ' \
               f'status={self.status()}, dst={self.change_action.dst_node.full_path}'
...
class UploadToGDriveCommand(CopyNodeCommand):
    ...
    def execute(self, cxt: CommandContext):
        ...
        if existing and existing.md5 == md5 and existing.get_size_bytes() == size_bytes:
            ...
            return CommandResult(CommandStatus.COMPLETED_NO_OP, to_upsert=[existing], to_delete=[self.change_action.dst_node])
        ...
        return CommandResult(CommandStatus.COMPLETED_OK, to_upsert=[goog_node], to_delete=[self.change_action.dst_node])
...
class DownloadFromGDriveCommand(CopyNodeCommand):
    ...
    def __repr__(self):
        return f'{__class__.__name__}(uid={self.identifier}, total_work={self.get_total_work()}, overwrite={self.overwrite}, ' \
               f'status={self.status()}, dst_node={self.change_action.dst_node}'
...
class CreateGDriveFolderCommand(Command):
    ...
    def __repr__(self):
        return f'{__class__.__name__}(uid={self.identifier}, total_work={self.get_total_work()}, ' \
               f'status={self.status()}, src_node={self.change_action.src_node}'
    ...
    def execute(self, cxt: CommandContext):
        ...
        if existing_src:
            ...
            if existing_dst:
                ...
                return CommandResult(CommandStatus.COMPLETED_NO_OP, to_upsert=[existing_dst], to_delete=[self.change_action.dst_node])
            else:
...
class DeleteGDriveFileCommand(DeleteNodeCommand):
    ...
    def execute(self, cxt: CommandContext):
        ...
        return CommandResult(CommandStatus.COMPLETED_OK, to_delete=[self.change_action.src_node])

    def __repr__(self):
        return f'{__class__.__name__}(uid={self.identifier}, total_work={self.get_total_work()}, to_trash={self.to_trash}, ' \
               f'status={self.status()}, model={self.change_action.src_node}'
