...
class CopyFileLocallyCommand(CopyNodeCommand):
    ...
    def __init__(self, uid: UID, change_action: ChangeAction, overwrite: bool = False):
        ...
        self.tag = f'{__class__.__name__}(uid={self.identifier}, ow={self.overwrite})'
    ...
    def execute(self, cxt: CommandContext):
        try:
            src_path = self.change_action.src_node.full_path
            dst_path = self.change_action.dst_node.full_path
            # TODO: what if staging dir is not on same file system?
            staging_path = os.path.join(cxt.staging_dir, self.change_action.dst_node.md5)
            ...
            if self.overwrite:
                file_util.copy_file_update(src_path=src_path, staging_path=staging_path,
                                           md5_expected=self.change_action.dst_node.md5, dst_path=dst_path,
                                           md5_src=self.change_action.src_node.md5, verify=True)
            else:
                file_util.copy_file_new(src_path=src_path, staging_path=staging_path, dst_path=dst_path,
                                        md5_src=self.change_action.dst_node.md5, verify=True)
            ...
            local_node = cxt.cache_manager.build_local_file_node(full_path=self.change_action.dst_node.full_path)
            cxt.cache_manager.add_or_update_node(local_node)
            self._status = CommandStatus.COMPLETED_NO_OP
        except Exception as err:
            # Try to log helpful info
            logger.exception(f'While copying file: src_path="{self.change_action.src_node.full_path}", dst_path="{self.change_action.dst_node.full_path}"')
    ...
    def __repr__(self):
        return f'{__class__.__name__}(uid={self.identifier}, total_work={self.get_total_work()}, overwrite={self.overwrite}, ' \
               f'status={self._status}, dst={self.change_action.dst_node}'
    ...
    def execute(self, cxt: CommandContext):
        try:
            ...
            if self.delete_empty_parent:
                parent_dir_path: str = str(pathlib.Path(self.change_action.src_node.full_path).parent)
            ...
            logger.exception(f'While deleting file: path={self.change_action.src_node.full_path}: {repr(err)}, to_trash={self.to_trash}')
            self._status = CommandStatus.STOPPED_ON_ERROR
            self._error = err

    def __repr__(self):
        return f'{__class__.__name__}(uid={self.identifier}, total_work={self.get_total_work()}, to_trash={self.to_trash}, ' \
               f'delete_empty_parent={self.delete_empty_parent}, status={self._status}, tgt_node={self.change_action.src_node}'
...
class MoveFileLocallyCommand(TwoNodeCommand):
    ...
    def execute(self, cxt: CommandContext):
        try:
            logger.debug(f'MV: src={self.change_action.src_node.full_path}')
            logger.debug(f'    dst={self.change_action.dst_node.full_path}')
            assert isinstance(self.change_action.src_node, LocalFileNode)
            assert isinstance(self.change_action.dst_node, LocalFileNode)
            file_util.move_file(self.change_action.src_node.full_path, self.change_action.dst_node.full_path)

            # Add to cache:
            local_node: LocalFileNode = cxt.cache_manager.build_local_file_node(full_path=self.change_action.dst_node.full_path)
            cxt.cache_manager.add_or_update_node(local_node)
            if not os.path.exists(self.change_action.src_node.full_path):
                self.change_action.src_node.set_exists(False)
                cxt.cache_manager.remove_node(local_node)
            else:
                logger.warning(f'Src node still exists after move: {self.change_action.src_node.full_path}')
            self._status = CommandStatus.COMPLETED_OK
        except Exception as err:
            logger.exception(f'While moving file: dest_path="{self.change_action.dst_node.full_path}", orig_path="'
                             f'{self.change_action.src_node.full_path}"')
            self._status = CommandStatus.STOPPED_ON_ERROR
            self._error = err

    def __repr__(self):
        return f'{__class__.__name__}(uid={self.identifier}, total_work={self.get_total_work()}, ' \
               f'status={self._status}, dst={self.change_action.dst_node.full_path}'
...
class CreatLocalDirCommand(Command):
    ...
    def execute(self, cxt: CommandContext):
        try:
            logger.debug(f'MKDIR: dst={self.change_action.src_node.full_path}')
            os.makedirs(name=self.change_action.src_node.full_path, exist_ok=True)

            # Add to cache:
            local_node = LocalDirNode(self.change_action.src_node.node_identifier, True)
            cxt.cache_manager.add_or_update_node(local_node)
            self._status = CommandStatus.COMPLETED_OK
        except Exception as err:
            logger.exception(f'While making local dir: dest_path="{self.change_action.src_node.full_path}"')
            self._status = CommandStatus.STOPPED_ON_ERROR
            self._error = err
...
class UploadToGDriveCommand(CopyNodeCommand):
    ...
    def execute(self, cxt: CommandContext):
        try:
            ...
            src_file_path = self.change_action.src_node.full_path
            md5 = self.change_action.src_node.md5
            size_bytes = self.change_action.src_node.get_size_bytes()

            existing, existing_raw = cxt.gdrive_client.get_existing_file_with_same_parent_and_name_and_criteria(self.change_action.dst_node,
                                                                                                                lambda x: True)
            if existing and existing.md5 == md5 and existing.get_size_bytes() == size_bytes:
                ...
                cxt.cache_manager.remove_node(self.change_action.dst_node)
            ...
            if self.overwrite:
                ...
                if existing:
                    ...
                    parent_goog_id: str = cxt.gdrive_client.resolve_parent_ids_to_goog_ids(self.change_action.dst_node)
            ...
            logger.exception(f'While uploading file to GDrive: dest_parent_ids="{self.change_action.dst_node.get_parent_uids()}", '
                             f'src_path="{self.change_action.src_node.full_path}"')
            self._status = CommandStatus.STOPPED_ON_ERROR
            self._error = err
...
class DownloadFromGDriveCommand(CopyNodeCommand):
    ...
    def execute(self, cxt: CommandContext):
        try:
            assert isinstance(self.change_action.src_node, GDriveFile), f'For {self.change_action.src_node}'
            src_goog_id = self.change_action.src_node.goog_id
            dst_path = self.change_action.dst_node.full_path

            if os.path.exists(dst_path):
                item: LocalFileNode = cxt.cache_manager.build_local_file_node(full_path=dst_path)
                if item and item.md5 == self.change_action.src_node.md5:
            ...
            try:
                ...
                raise
            staging_path = os.path.join(cxt.staging_dir, self.change_action.src_node.md5)
            ...
            if item.md5 != self.change_action.src_node.md5:
                raise RuntimeError(f'Downloaded MD5 ({item.md5}) does not matched expected ({self.change_action.src_node.md5})!')
            ...
            logger.exception(f'While downloading file from GDrive: dest_path="{self.change_action.dst_node.full_path}", '
                             f'src_goog_id="{self.change_action.src_node.goog_id}"')
            self._status = CommandStatus.STOPPED_ON_ERROR
            self._error = err

    def __repr__(self):
        return f'{__class__.__name__}(uid={self.identifier}, total_work={self.get_total_work()}, overwrite={self.overwrite}, ' \
               f'status={self._status}, dst_node={self.change_action.dst_node}'
...
class CreateGDriveFolderCommand(Command):
    ...
    def execute(self, cxt: CommandContext):
        try:
            assert isinstance(self.change_action.src_node, GDriveNode) and self.change_action.src_node.is_dir(), \
                f'For {self.change_action.src_node}'

            parent_goog_id: str = cxt.gdrive_client.resolve_parent_ids_to_goog_ids(self.change_action.src_node)
            name = self.change_action.src_node.name
            ...
            if len(existing.nodes) > 0:
                ...
                goog_node.uid = self.change_action.src_node.uid
            else:
                goog_node = cxt.gdrive_client.create_folder(name=self.change_action.src_node.name,
                                                            parent_goog_ids=[parent_goog_id], uid=self.change_action.src_node.uid)
            ...
            logger.exception(f'While creating folder on GDrive: name="{self.change_action.src_node.name}", '
                             f'parent_uids="{self.change_action.src_node.get_parent_uids()}"')
            self._status = CommandStatus.STOPPED_ON_ERROR
            self._error = err

    def __repr__(self):
        return f'{__class__.__name__}(uid={self.identifier}, total_work={self.get_total_work()}, ' \
               f'status={self._status}, src_node={self.change_action.src_node}'
...
class MoveFileGDriveCommand(TwoNodeCommand):
    ...
    def execute(self, cxt: CommandContext):
        try:
            assert isinstance(self.change_action.dst_node, GDriveFile), f'For {self.change_action.dst_node}'
            assert isinstance(self.change_action.src_node, GDriveFile), f'For {self.change_action.src_node}'
            # this requires that any parents have been created and added to the in-memory cache (and will fail otherwise)
            src_parent_goog_id: str = cxt.gdrive_client.resolve_parent_ids_to_goog_ids(self.change_action.src_node)
            dst_parent_goog_id: str = cxt.gdrive_client.resolve_parent_ids_to_goog_ids(self.change_action.dst_node)
            src_goog_id = self.change_action.src_node.goog_id
            assert not self.change_action.dst_node.goog_id
            dst_name = self.change_action.dst_node.name

            existing_src, raw = cxt.gdrive_client.get_existing_file_with_same_parent_and_name_and_criteria(self.change_action.src_node,
            ...
            if existing_src:
                ...
                assert goog_node.name == self.change_action.dst_node.name and goog_node.uid == self.change_action.src_node.uid
                ...
                existing_dst, raw = cxt.gdrive_client.get_existing_file_with_same_parent_and_name_and_criteria(self.change_action.dst_node,
                                                                                                          lambda x: x.goog_id == src_goog_id)
                if existing_dst:
                    ...
                    assert existing_dst.uid == self.change_action.src_node.uid and existing_dst.goog_id == self.change_action.src_node.goog_id,\
                        f'For {existing_dst} and {self.change_action.src_node}'
                    ...
                    raise RuntimeError(f'Could not find expected node in source or dest locations. Looks like the model is out of date '
                                       f'(goog_id={self.change_action.src_node.goog_id})')

        except Exception as err:
            logger.exception(f'While moving file within GDrive: dest_parent_ids="{self.change_action.dst_node.get_parent_uids()}", '
                             f'src_node="{self.change_action.src_node}"')
            self._status = CommandStatus.STOPPED_ON_ERROR
            self._error = err
...
class DeleteGDriveFileCommand(DeleteNodeCommand):
    ...
    def execute(self, cxt: CommandContext):
        try:
            assert isinstance(self.change_action.src_node, GDriveFile)
            tgt_goog_id = self.change_action.src_node.goog_id

            existing = cxt.gdrive_client.get_existing_file_with_same_parent_and_name_and_criteria(self.change_action.src_node,
                                                                                                  lambda x: x.goog_id == tgt_goog_id)
            if not existing:
            ...
            if self.to_trash:
                cxt.gdrive_client.trash(self.change_action.src_node.goog_id)
                self.change_action.src_node.trashed = EXPLICITLY_TRASHED
            else:
                cxt.gdrive_client.hard_delete(self.change_action.src_node.goog_id)

            cxt.cache_manager.remove_node(self.change_action.src_node, self.to_trash)
            self._status = CommandStatus.COMPLETED_OK
        except Exception as err:
            logger.exception(f'While deleting from GDrive (to_trash={self.to_trash}, node={self.change_action.src_node}"')
            self._status = CommandStatus.STOPPED_ON_ERROR
            self._error = err

    def __repr__(self):
        return f'{__class__.__name__}(uid={self.identifier}, total_work={self.get_total_work()}, to_trash={self.to_trash}, ' \
               f'status={self._status}, model={self.change_action.src_node}'
