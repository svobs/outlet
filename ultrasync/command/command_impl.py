...
from command.command_interface import Command, CommandContext, CommandStatus, CopyNodeCommand, TwoNodeCommand
...
class CopyFileLocallyCommand(CopyNodeCommand):
    ...
    def execute(self, context: CommandContext):
        try:
            ...
            staging_path = os.path.join(context.staging_dir, self.tgt_node.md5)
            ...
            local_node = context.cache_manager.build_local_file_node(full_path=self.tgt_node.full_path)
            context.cache_manager.add_or_update_node(local_node)
...
class UploadToGDriveCommand(TwoNodeCommand):
    """
    Copy Local -> GDrive
    """
    def __init__(self, uid: UID, change_action: ChangeAction, tgt_node: DisplayNode, src_node: DisplayNode):
    ...
    def execute(self, context: CommandContext):
        try:
            ...
            parent_goog_id: str = context.resolve_parent_ids_to_goog_ids(self.tgt_node)
                ...
                if len(existing.nodes) > 0:
                    for existing_data, existing_node in zip(existing.raw_items, existing.nodes):
                        ...
                        if existing_node.md5 == new_md5 and existing_node.get_size_bytes() == new_size:
                            self._status = CommandStatus.COMPLETED_NO_OP
                            # Update cache manager - it's likely out of date:
                            cached_node = context.cache_manager.get_goog_node(parent_uid=self.tgt_node.get_parent_uids()[0], goog_id=existing_node.goog_id)
                            if cached_node:
                                ...
                                if existing_node == cached_node:
                                    ...
                                    if cached_node.uid != self.tgt_node.uid:
                                        ...
                                        context.cache_manager.remove_node(self.tgt_node, False)
                                    return
                                # Fall through! Wheee!
    ...
    def execute(self, context: CommandContext):
        try:
            ...
            try:
                ...
                raise
            staging_path = os.path.join(context.staging_dir, self.src_node.md5)
    ...
    def execute(self, context: CommandContext):
        try:
            ...
            if len(existing.nodes) > 0:
                ...
                goog_node = context.gdrive_client.create_folder(name=self.tgt_node.name,
                                                                parent_goog_ids=[parent_goog_id], uid=self.tgt_node.uid)
...
class MoveFileGDriveCommand(TwoNodeCommand):
    ...
    def execute(self, context: CommandContext):
        try:
            ...
            assert isinstance(self.src_node, GoogFile), f'For {self.src_node}'
            # this requires that any parents have been created and added to the in-memory cache (and will fail otherwise)
            dst_parent_goog_id: str = context.resolve_parent_ids_to_goog_ids(self.tgt_node)
            src_parent_goog_id: str = context.resolve_parent_ids_to_goog_ids(self.src_node)
            ...
            if src_node_found:
                context.gdrive_client.modify_meta(goog_id=self.src_node.goog_id, uid=self.tgt_node.uid,
...
class DeleteGDriveFileCommand(Command):
    ...
    def execute(self, context: CommandContext):
        try:
            ...
            parent_goog_id: str = context.resolve_parent_ids_to_goog_ids(self.tgt_node)
            # did not find the target file; see if our operation was already completed
            existing = context.gdrive_client.get_existing_file_with_parent_and_name(parent_goog_id=parent_goog_id, name=self.tgt_node.name)
            ...
            if self.to_trash:
                context.gdrive_client.trash(self.tgt_node.goog_id)
                self.tgt_node.trashed = EXPLICITLY_TRASHED
            else:
                context.gdrive_client.hard_delete(self.tgt_node.goog_id)

            context.cache_manager.remove_node(self.tgt_node, self.to_trash)
            self._status = CommandStatus.COMPLETED_OK
        except Exception as err:
