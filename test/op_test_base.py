...
def get_name_lower(display_node: TNode):
...
class OpTestBase(unittest.TestCase):
    ...
    def do_setup(self, do_before_verify_func: Callable = None):
        ...
        def after_left_tree_loaded(sender):
            logger.debug(f'Received signal: "{Signal.POPULATE_UI_TREE_DONE.name}" for "{sender}"')
            load_left_done.set()
        ...
        dispatcher.connect(signal=Signal.POPULATE_UI_TREE_DONE.name, sender=ID_LEFT_TREE, receiver=after_left_tree_loaded)
        dispatcher.connect(signal=Signal.POPULATE_UI_TREE_DONE.name, sender=ID_RIGHT_TREE, receiver=after_right_tree_loaded)
    ...
    def verify_one_memstore_dir(self, tree_con, expected: List[FNode], actual: Iterable[TNode],
                                dir_deque: Deque[Tuple[List[FNode], Iterable[TNode]]]):
        actual_iter = iter(actual)
        for i in range(0, len(expected)):
            expected_node: FNode = expected[i]
            try:
                actual_node: TNode = next(actual_iter)
                ...
                if expected_node.is_dir():
                    ...
                    actual_list: Iterable[TNode] = tree_con.get_tree().get_children(actual_node)
    ...
    def verify_one_display_dir(self, tree_con, expected: List[FNode], actual: List[TNode],
                               dir_deque: Deque[Tuple[List[FNode], Iterable[TNode]]]):
        ...
        for i in range(0, len(expected)):
            ...
            try:
                ...
                if expected_node.is_dir():
                    assert isinstance(expected_node, DNode)
                    expected_list: List[FNode] = expected_node.children
                    actual_list: Iterable[TNode] = tree_con.display_store.get_displayed_children_of(actual_node.uid)
                    if actual_list:
                        dir_deque.append((expected_list, actual_list))
    ...
    def verify(self, tree_con: TreePanelController, expected_list_root: List[FNode]):
        ...
        dir_deque: Deque[Tuple[List[FNode], Iterable[TNode]]] = collections.deque()
        ...
        actual_list: Iterable[TNode] = backing_tree.get_children_for_root()
        ...
        while len(dir_deque) > 0:
            ...
            actual_list: List[TNode] = list(actual_list)
        ...
        count_dir = 0
        actual_list: List[TNode] = tree_con.display_store.get_displayed_children_of(None)
        dir_deque.append((expected_list_root, actual_list))
        while len(dir_deque) > 0:
            count_dir += 1
            expected_list, actual_list = dir_deque.popleft()
    ...
    def find_node_by_name(self, tree_con, node_name: str) -> SPIDNodePair:
        ...
        sn = tree_con.display_store.get_node_data(tree_iter)
